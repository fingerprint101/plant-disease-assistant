...
\author{
\IEEEauthorblockN{Plant Disease Assistant Project Team}
\IEEEauthorblockA{
...
strongest tested corruption, while Grad-CAM obtains 24.2\% mean top-quantile IoU with the annotated
lesions. The final cross-domain values will complete the transfer analysis. These
...
metadata. EXIF orientation is applied consistently to image/mask pairs; eight orientation cases were
identified. PlantSeg supplies all model fitting, the official holdout, synthetic robustness images,
and explanation ground truth.
...
The complete KDD process is represented as follows. Problem understanding defines field-image
disease screening, functional requirements, and the boundary between assistance and expert
diagnosis. Data selection assigns PlantSeg, PlantVillage, and PlantDoc distinct roles. Cleaning
verifies checksums and decoding, audits masks and boxes, identifies split duplicates, and rejects
unsupported taxonomy equivalences. Transformation applies orientation, binarizes masks, derives
YOLO boxes, creates matched domain views, resizes and normalizes inputs, and generates predicted
lesion crops. Modeling trains two YOLO variants and three classifiers. Evaluation combines holdout,
cross-validation, localization, calibration, domain-shift, corruption, and explanation metrics.
Finally, the prototype exposes the selected inference path through an upload-oriented interface.
...
\midrule
Disease-aware YOLO11n & 61.37 & 53.32 & 49.60 & 86.48 & 26.3 \\
Lesion YOLO + baseline CNN & 41.32 & 29.04 & 29.99 & 100 & 28.7 \\
Lesion YOLO + EfficientNetB0 & 66.69 & 59.02 & 58.69 & 100 & 24.5 \\
Lesion YOLO + MobileNetV3-Large & \textbf{66.75} & 58.17 & 58.69 & 100 & 24.3 \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{tabular}{lccccc}
\toprule
Classifier pipeline & PlantSeg macro F1 & PlantVillage macro F1 & Change & PlantSeg ECE & PlantVillage ECE \\
\midrule
...
\begin{table*}[t]
\caption{Clean and severity-5 accuracy (\%). Parentheses contain the absolute change from
the clean condition.}
...
\midrule
Baseline CNN & 40.0 & 12.5 (-27.5) & 17.8 (-22.2) & 26.1 (-13.8) & 25.8 (-14.2) & 34.5 (-5.4) & 35.5 (-4.5) \\
EfficientNetB0 & 67.3 & 29.7 (-37.7) & 49.6 (-17.7) & 62.0 (-5.3) & 48.5 (-18.8) & 63.2 (-4.2) & 63.6 (-3.7) \\
MobileNetV3-Large & 66.5 & 30.5 (-36.0) & 46.8 (-19.7) & 61.8 (-4.7) & 45.7 (-20.8) & 61.3 (-5.2) & 63.8 (-2.7) \\
\bottomrule
\end{tabular}}
\end{table*}

Gaussian blur is the most damaging transformation for every model. At severity 5, EfficientNetB0
falls from 67.3\% to 29.7\% accuracy and MobileNetV3-Large falls from 66.5\% to 30.5\%. Brightness
and JPEG compression cause intermediate degradation. Crop and occlusion are comparatively well
...
as the principal tested acquisition weakness.
...
the smallest model by far (2.66M parameters, 5.6~MB checkpoint) and reaches 26.3 images/s mean
throughput (18.1~ms median latency), but its 95th-percentile latency is far higher (231~ms),
indicating occasional slow outlier images rather than uniformly fast inference. The baseline CNN
is the fastest cascade at 28.7 images/s (3.0M parameters, 10.5~MB), and
the two pretrained classifiers are close to each other and slightly slower (EfficientNetB0: 24.5
images/s, 6.7M parameters, 55.8~MB; MobileNetV3-Large: 24.3 images/s, 6.9M parameters, 58.0~MB).
The throughput ranking does not track accuracy: the fastest system (baseline CNN) is also the
least accurate, and MobileNetV3-Large trades a marginal throughput cost for a 29-point higher
macro F1 than the baseline CNN. All four systems remain comfortably above real-time single-image
...
86.8\% to 43.2\%. Severe blur is the largest tested corruption weakness, and Grad-CAM places 41.5\%
of its activation energy inside lesions occupying 21.3\% of the image on average.
